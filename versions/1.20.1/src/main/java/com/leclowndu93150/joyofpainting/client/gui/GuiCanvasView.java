...
public class GuiCanvasView extends Screen {
    ...
    public GuiCanvasView(ItemStack canvasStack, Component title, CanvasType canvasType, EntityEasel easel) {
        ...
        if (NbtCanvas.hasPixels(canvasStack)) {
            this.pixels = NbtCanvas.getPixels(canvasStack);
            this.authorName = NbtCanvas.getAuthor(canvasStack);
            this.canvasTitle = NbtCanvas.getTitle(canvasStack);
            this.generation = NbtCanvas.getGeneration(canvasStack);
        }
        ...
        if (NbtCanvas.hasSidePixels(canvasStack)) {
            int[] sp = NbtCanvas.getSidePixels(canvasStack);
            if (sp.length == CanvasSides.count(canvasType)) {
                this.sidePixels = sp;
            }
        }
    }
