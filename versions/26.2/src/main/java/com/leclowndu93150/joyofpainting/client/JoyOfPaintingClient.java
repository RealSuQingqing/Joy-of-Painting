    ...
    public static void showCanvasGui(Player player) {
        ...
        if (heldItem.getItem() instanceof ItemCanvas itemCanvas) {
            ...
            if (offhandItem.isEmpty() || !(offhandItem.getItem() instanceof ItemPalette) || gen > 0) {
                minecraft.gui.setScreen(new GuiCanvasView(heldItem, Component.translatable("item.joyofpainting.item_canvas"), itemCanvas.getCanvasType(), null));
            } else {
                minecraft.gui.setScreen(new GuiCanvasEdit(minecraft.player, heldItem, offhandItem, Component.translatable("item.joyofpainting.item_canvas"), itemCanvas.getCanvasType(), null));
            }
            ...
            if (offhandItem.isEmpty() || !(offhandItem.getItem() instanceof ItemCanvas offCanvas)) {
                minecraft.gui.setScreen(new GuiPalette(heldItem, Component.translatable("item.joyofpainting.item_palette")));
            } else {
                int gen = offhandItem.getOrDefault(ModDataComponents.CANVAS_GENERATION.get(), 0);
                if (gen > 0) {
                    minecraft.gui.setScreen(new GuiCanvasView(offhandItem, Component.translatable("item.joyofpainting.item_canvas"), offCanvas.getCanvasType(), null));
                } else {
                    minecraft.gui.setScreen(new GuiCanvasEdit(minecraft.player, offhandItem, heldItem, Component.translatable("item.joyofpainting.item_canvas"), offCanvas.getCanvasType(), null));
                }
            }
        }
    }
