    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float f) {
        ...
        if (sidesActive) {
            ...
            for (int k = 0; k < canvasPixelWidth; k++) {
                ...
                graphics.fill(x, canvasY - scale, x + scale, canvasY, getSidePixel(CanvasSides.topOffset() + k));
                graphics.fill(x, canvasY + canvasPixelHeight * scale, x + scale, canvasY + canvasPixelHeight * scale + scale, getSidePixel(CanvasSides.bottomOffset(canvasType) + k));
            }
            for (int i = 0; i < canvasPixelHeight; i++) {
                int y = canvasY + i * scale;
                graphics.fill(canvasX - scale, y, canvasX, y + scale, getSidePixel(CanvasSides.leftOffset(canvasType) + i));
                graphics.fill(canvasX + canvasWidth, y, canvasX + canvasWidth + scale, y + scale, getSidePixel(CanvasSides.rightOffset(canvasType) + i));
            }
        }
        ...
        if (generation > 0 && !canvasTitle.isEmpty()) {
            ...
            graphics.fill((int) (minX - 10), canvasY - 30, (int) (maxX + 10), canvasY - 4, 0xFFEEEEEE);
            graphics.text(font, title, (int) titleX, canvasY - 25, 0xFF111111, false);
            graphics.text(font, gen, (int) genX, canvasY - 14, 0xFF444444, false);
        }
    }
